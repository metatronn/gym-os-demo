import {
  Html,
  Head,
  Body,
  Container,
  Heading,
  Text,
  Button,
  Section,
  Row,
  Column,
  Hr,
} from "@react-email/components";
import * as React from "react";

interface DailyDigestEmailProps {
  gymName: string;
  date: string;
  stats: {
    newLeads: number;
    newMembers: number;
    revenue: number;
  };
}

const bg = "#0A0F1C";
const text = "#F1F5F9";
const muted = "#94A3B8";
const accent = "#0350FF";
const cardBg = "#111827";

function formatDollars(cents: number): string {
  return `$${(cents / 100).toFixed(2)}`;
}

function StatCard({ label, value }: { label: string; value: string }) {
  return (
    <Column style={{ textAlign: "center" as const, padding: "16px 8px" }}>
      <Text
        style={{
          color: text,
          fontSize: 28,
          fontWeight: 700,
          margin: 0,
          lineHeight: "32px",
        }}
      >
        {value}
      </Text>
      <Text
        style={{
          color: muted,
          fontSize: 12,
          margin: "4px 0 0 0",
          textTransform: "uppercase" as const,
          letterSpacing: "0.05em",
        }}
      >
        {label}
      </Text>
    </Column>
  );
}

export default function DailyDigestEmail({
  gymName = "My Gym",
  date = "March 29, 2026",
  stats = { newLeads: 5, newMembers: 2, revenue: 34500 },
}: DailyDigestEmailProps) {
  return (
    <Html>
      <Head />
      <Body
        style={{
          backgroundColor: bg,
          fontFamily: "sans-serif",
          margin: 0,
          padding: 0,
        }}
      >
        <Container
          style={{ maxWidth: 560, margin: "0 auto", padding: "40px 24px" }}
        >
          <Heading
            style={{
              color: text,
              fontSize: 28,
              fontWeight: 700,
              marginBottom: 8,
            }}
          >
            Daily Digest
          </Heading>
          <Text style={{ color: muted, fontSize: 16, marginBottom: 24 }}>
            <strong style={{ color: text }}>{gymName}</strong> — {date}
          </Text>

          <Hr style={{ borderColor: "#1E293B", margin: "24px 0" }} />

          <Section
            style={{
              backgroundColor: cardBg,
              borderRadius: 8,
              padding: "8px 0",
            }}
          >
            <Row>
              <StatCard label="New Leads" value={String(stats.newLeads)} />
              <StatCard label="New Members" value={String(stats.newMembers)} />
              <StatCard label="Revenue" value={formatDollars(stats.revenue)} />
            </Row>
          </Section>

          <Section style={{ marginTop: 32, textAlign: "center" as const }}>
            <Button
              href="https://app.gymos.ai/dashboard"
              style={{
                backgroundColor: accent,
                color: "#FFFFFF",
                fontSize: 16,
                fontWeight: 600,
                padding: "12px 32px",
                borderRadius: 6,
                textDecoration: "none",
              }}
            >
              View Dashboard
            </Button>
          </Section>

          <Text
            style={{
              color: muted,
              fontSize: 12,
              marginTop: 40,
              textAlign: "center" as const,
            }}
          >
            GYM OS - The AI-native operating system for gyms
          </Text>
        </Container>
      </Body>
    </Html>
  );
}
